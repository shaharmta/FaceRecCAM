#!/usr/bin/env python3
"""
Debug script to check and clear the Pinecone database
"""

import pinecone_db

def debug_database():
    print("=== DEBUGGING PINECONE DATABASE ===")
    
    try:
        # List all people
        people = pinecone_db.list_people()
        print(f"People in database: {len(people)}")
        
        if people:
            print("\nPeople details:")
            for person in people:
                print(f"  - Person ID: {person['id']}")
                print(f"    Vectors: {person['vector_count']}")
                print(f"    Last seen: {person['last_seen']}")
                print()
        else:
            print("Database is empty!")
            
        # Test get_next_person_id
        print("\n=== TESTING get_next_person_id ===")
        next_id = pinecone_db.get_next_person_id()
        print(f"Next person ID would be: {next_id}")
            
    except Exception as e:
        print(f"Error: {e}")

def clear_database():
    print("\n=== CLEARING DATABASE ===")
    try:
        index = pinecone_db.index
        index.delete(delete_all=True)
        print("Database cleared successfully!")
    except Exception as e:
        print(f"Error clearing database: {e}")

if __name__ == "__main__":
    debug_database()
    
    response = input("\nDo you want to clear the database? (y/n): ")
    if response.lower() == 'y':
        clear_database()
        print("\nDatabase after clearing:")
        debug_database()
